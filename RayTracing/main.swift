...
let left   = Metal(albedo: ColorF(x: 0.8, y: 0.8, z: 0.8), fuzz: 0)
let right  = Metal(albedo: ColorF(x: 0.8, y: 0.6, z: 0.2), fuzz: 0.2)

private let world: [any Hittable] = [
    Sphere(center: Point3D(x:    0, y: -100.5, z: -1  ), radius: 100.0, material: ground),
    Sphere(center: Point3D(x:    0, y:      0, z: -1.3), radius:   0.5, material: center),
